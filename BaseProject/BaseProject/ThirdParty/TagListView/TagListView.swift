...
@objc public protocol TagListViewDelegate {
    @objc optional func tagRefresh(_ height: CGFloat) -> Void
...
open class TagListView: UIView {
    ...
    func setTagListViewHeight(_ height: Int) {
    ...
    private func rearrangeViews() {
        ...
        rows = currentRow
        if ml_refresh == true {
            delegate?.tagRefresh!(CGFloat(rows)*(tagViewHeight+10))
        }
        ml_refresh = false
        invalidateIntrinsicContentSize()
    }
